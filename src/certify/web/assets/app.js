...
const formatDate = value => value ? new Intl.DateTimeFormat("de-DE", { dateStyle: "medium", timeStyle: "short" }).format(new Date(`${value}${value.endsWith("Z") ? "" : "Z"}`)) : "–";
...
async function login(event) {
  ...
  try {
    ...
    state.token = result.access_token; sessionStorage.setItem("certify_token", state.token); await bootstrap();
...
async function bootstrap() {
  ...
  renderNav(); await navigate(location.hash.slice(1) || "dashboard"); checkHealth();
}
function renderNav() {
...
async function users() {
  ...
  $("#content").innerHTML = `${intro("Benutzer", "Personalisierte Zugänge und rollenbasierte Berechtigungen.", `<button class="button primary" data-action="new-user">＋ Benutzer</button>`)}<section class="panel"><div class="table-wrap"><table class="data-table"><thead><tr><th>Benutzer</th><th>Rolle</th><th>E-Mail</th><th>Benachrichtigung</th><th>Status</th><th>Erstellt</th></tr></thead><tbody>${state.users.map(item => `<tr><td class="cell-main">${esc(item.username)}</td><td>${badge(item.role)}</td><td>${esc(item.email || "–")}</td><td>${esc(item.notify_level)}</td><td>${badge(item.active ? "active" : "inactive")}</td><td>${formatDate(item.created_at)}</td></tr>`).join("")}</tbody></table></div></section>`;
}
function newUser() {
  openModal("Benutzer anlegen", "Identität", `<form id="user-form" class="grid-form"><label>Benutzername<input name="username" required pattern="[a-zA-Z0-9_.@-]+"></label><label>Rolle<select name="role"><option value="operator">Operator</option><option value="auditor">Auditor</option><option value="admin">Administrator</option></select></label><label class="full">E-Mail<input name="email" type="email"></label><label class="full">Initiales Passwort<input name="password" type="password" minlength="14" required><span class="muted">Mindestens 14 Zeichen mit Groß-/Kleinbuchstaben, Zahl und Sonderzeichen.</span></label><div class="modal-actions full"><button type="button" class="button" data-close>Abbrechen</button><button class="button primary">Benutzer anlegen</button></div></form>`);
  $("#user-form").onsubmit = async event => { event.preventDefault(); const raw = formJson(event.target); if (!raw.email) raw.email = null; try { await api("/api/v1/users", { method: "POST", body: JSON.stringify(raw) }); closeModal(); toast("Benutzer wurde angelegt."); navigate("users"); } catch (error) { toast(error.message, true); } };
}
...
async function settings() {
  $("#content").innerHTML = `${intro("Konto & Sicherheit", "Persönliche Kontodaten und Zugangsschutz.")}<div class="dashboard-grid"><section class="panel"><header><h3>Kontodetails</h3></header><div class="panel-body"><dl class="detail-list"><dt>Benutzername</dt><dd>${esc(state.me.username)}</dd><dt>Rolle</dt><dd>${roleLabel(state.me.role)}</dd><dt>E-Mail</dt><dd>${esc(state.me.email || "–")}</dd><dt>Konto erstellt</dt><dd>${formatDate(state.me.created_at)}</dd></dl></div></section><section class="panel"><header><h3>Passwort ändern</h3></header><div class="panel-body"><form id="password-form"><label>Aktuelles Passwort<input name="current_password" type="password" required></label><label>Neues Passwort<input name="new_password" type="password" minlength="14" required></label><button class="button primary">Passwort aktualisieren</button></form></div></section></div>`;
  $("#password-form").onsubmit = async event => { event.preventDefault(); try { await api("/api/v1/users/me/password", { method: "PUT", body: JSON.stringify(formJson(event.target)) }); event.target.reset(); toast("Passwort wurde geändert."); } catch (error) { toast(error.message, true); } };
}
