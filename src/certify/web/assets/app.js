...
async function settings() {
  ...
  if ($("#totp-setup")) $("#totp-setup").onclick = async () => { try { const result = await api("/api/v1/users/me/totp/setup", { method: "POST" }); openModal("TOTP einrichten", "Zwei-Faktor-Authentifizierung", `<p class="muted">Hinterlegen Sie diesen Schlüssel oder die URI in Ihrer Authenticator-App und bestätigen Sie anschließend den angezeigten Code.</p><label>Geheimer Schlüssel</label><div class="code-box">${esc(result.secret)}</div><label>Authenticator-URI</label><div class="code-box">${esc(result.otpauth_uri)}</div><form id="totp-confirm-form"><label>Sechsstelliger Code<input name="code" inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" required></label><div class="modal-actions"><button class="button primary">TOTP aktivieren</button></div></form>`); $("#totp-confirm-form").onsubmit = async event => { event.preventDefault(); try { await api("/api/v1/users/me/totp/confirm", { method: "POST", body: JSON.stringify(formJson(event.target)) }); state.me.totp_enabled = 1; closeModal(); toast("TOTP wurde aktiviert."); settings(); } catch (error) { toast(error.message, true); } }; } catch (error) { toast(error.message, true); } };
}
