...
const state = { token: sessionStorage.getItem("certify_token"), me: null, certificates: [], targets: [], caAccounts: [], users: [], keys: [], audit: [] };
const pages = [
  ...
  { id: "authorities", label: "Zertifizierungsstellen", icon: "◎", roles: ["admin", "operator", "auditor"] },
...
async function loadCore() {
  const tasks = [api("/api/v1/certificates"), api("/api/v1/ca-accounts")];
  ...
  [state.certificates, state.caAccounts, state.targets, state.users] = await Promise.all(tasks);
}
const statusLabel = value => ({ pending: "In Vorbereitung", validating: "Domainprüfung", issued: "Ausgestellt", failed: "Fehlgeschlagen", revoked: "Widerrufen" })[value] || value;
const certRows = certificates => certificates.map(cert => `<tr><td><div class="cell-main">${esc(cert.common_name)}</div><div class="cell-sub">${esc(cert.sans.join(", ") || "Keine SANs")}</div></td><td>${badge(cert.status)}<div class="cell-sub">${esc(statusLabel(cert.status))}</div></td><td>${esc(cert.ca_name || "Direktes ACME-Verzeichnis")}</td><td>${esc(cert.challenge)}</td><td>${cert.targets.map(target => esc(target.name)).join(", ") || "–"}</td><td><div class="table-actions"><button class="small-button" data-cert-detail="${cert.id}">Status & Details</button>${["admin", "operator"].includes(state.me.role) ? `<button class="small-button" data-cert-key="${cert.id}">Schlüssel</button>` : ""}</div></td></tr>`).join("");
...
function newCertificate() {
  if (!state.caAccounts.some(item => item.enabled)) { toast("Richten Sie zuerst eine Zertifizierungsstelle ein.", true); navigate("authorities"); return; }
  const accounts = state.caAccounts.filter(item => item.enabled);
  openModal("Zertifikat anfordern", "Geführter Antrag", `<form id="certificate-form"><ol class="wizard-steps"><li class="active">1 · Namen</li><li>2 · Validierung</li><li>3 · Schlüssel & Ziele</li><li>4 · Prüfen</li></ol>
  <section class="wizard-page active" data-step="0"><h3>Für welche Namen soll das Zertifikat gelten?</h3><p class="muted">Der Common Name ist der primäre DNS-Name. Weitere Namen können Sie kommasepariert ergänzen.</p><label>Common Name<input name="common_name" placeholder="service.example.org" required></label><label>Subject Alternative Names <span class="optional">optional</span><input name="sans" placeholder="www.example.org, api.example.org"></label></section>
  <section class="wizard-page" data-step="1"><h3>Zertifizierungsstelle und Domainprüfung</h3><p class="muted">Die CA prüft über die gewählte Challenge, ob Sie die Domain kontrollieren.</p><label>Zertifizierungsstelle<select name="ca_account_id" required>${accounts.map(item => `<option value="${item.id}" data-directory="${esc(item.directory_url)}">${esc(item.name)} · ${esc(item.email)}</option>`).join("")}</select></label><input name="acme_directory" type="hidden" value="${esc(accounts[0].directory_url)}"><label>Challenge<select name="challenge"><option value="dns-01">DNS-01 · empfohlen für Wildcards und interne Automatisierung</option><option value="http-01">HTTP-01 · Datei über Port 80 bereitstellen</option></select></label><div class="info-box"><strong>Was passiert danach?</strong><span>Certify erstellt den ACME-Auftrag, veröffentlicht die Challenge und wartet auf die Bestätigung der CA.</span></div></section>
  <section class="wizard-page" data-step="2"><h3>Schlüssel und Bereitstellung</h3><label>Schlüsselmodus<select name="key_mode"><option value="managed">Von Certify sicher erzeugen und verwalten</option><option value="csr">Vorhandene CSR verwenden</option></select></label><label>CSR <span class="optional">nur bei externer CSR</span><textarea name="csr_pem" placeholder="-----BEGIN CERTIFICATE REQUEST-----"></textarea></label><div><label>Zielsysteme <span class="optional">optional</span></label><div class="check-list">${state.targets.length ? state.targets.map(target => `<label><input type="checkbox" name="target_ids" value="${target.id}">${esc(target.name)} · ${esc(target.adapter)}</label>`).join("") : `<span class="muted">Noch keine Zielsysteme eingerichtet. Das Zertifikat kann später zugewiesen werden.</span>`}</div></div></section>
  <section class="wizard-page" data-step="3"><h3>Antrag prüfen</h3><p class="muted">Nach dem Absenden sehen Sie jeden Verarbeitungsschritt in der Statusansicht. Ein Antrag stellt nicht sofort ein Zertifikat aus.</p><div id="certificate-review" class="review-box"></div><label class="consent"><input type="checkbox" required> Ich bestätige, dass ich zur Anforderung für diese Domains berechtigt bin.</label></section>
  <div class="modal-actions"><button type="button" class="button" id="wizard-back" hidden>Zurück</button><button type="button" class="button primary" id="wizard-next">Weiter</button><button class="button primary" id="wizard-submit" hidden>Auftrag verbindlich anlegen</button></div></form>`);
  let step = 0; const form = $("#certificate-form");
  const showStep = next => { step = next; $$(".wizard-page", form).forEach((item, index) => item.classList.toggle("active", index === step)); $$(".wizard-steps li", form).forEach((item, index) => item.classList.toggle("active", index <= step)); $("#wizard-back").hidden = step === 0; $("#wizard-next").hidden = step === 3; $("#wizard-submit").hidden = step !== 3; if (step === 3) { const data = formJson(form); const account = accounts.find(item => item.id === Number(data.ca_account_id)); $("#certificate-review").innerHTML = `<dl class="detail-list"><dt>Domain</dt><dd>${esc(data.common_name)}</dd><dt>Weitere Namen</dt><dd>${esc(data.sans || "–")}</dd><dt>CA-Konto</dt><dd>${esc(account?.name || "–")}</dd><dt>Challenge</dt><dd>${esc(data.challenge)}</dd><dt>Schlüssel</dt><dd>${esc(data.key_mode)}</dd></dl>`; } };
  $("#wizard-next").onclick = () => { const fields = $$('[data-step="' + step + '"] input, [data-step="' + step + '"] select, [data-step="' + step + '"] textarea', form); if (fields.some(field => !field.reportValidity())) return; if (step === 2 && form.elements.key_mode.value === "csr" && !form.elements.csr_pem.value.trim()) { toast("Bitte fügen Sie die externe CSR ein.", true); return; } showStep(step + 1); };
  $("#wizard-back").onclick = () => showStep(step - 1);
  form.elements.ca_account_id.onchange = event => { form.elements.acme_directory.value = accounts.find(item => item.id === Number(event.target.value)).directory_url; };
  form.onsubmit = async event => { event.preventDefault(); const raw = formJson(form); raw.ca_account_id = Number(raw.ca_account_id); raw.sans = raw.sans.split(",").map(item => item.trim()).filter(Boolean); raw.target_ids = $$('[name="target_ids"]:checked', form).map(item => Number(item.value)); if (!raw.csr_pem) raw.csr_pem = null; delete raw[""]; try { const result = await api("/api/v1/certificates", { method: "POST", body: JSON.stringify(raw) }); closeModal(); toast(`Auftrag #${result.id} wurde angelegt und wird vorbereitet.`); navigate("certificates"); } catch (error) { toast(error.message, true); } };
}
function showCertificate(id) {
  const cert = state.certificates.find(item => item.id === id); if (!cert) return;
  const stage = ({ pending: 1, validating: 2, issued: 4, failed: 2, revoked: 4 })[cert.status] || 1;
  openModal(cert.common_name, `Auftrag #${cert.id} · ${statusLabel(cert.status)}`, `<div class="status-summary"><div>${badge(cert.status)}<h3>${esc(statusLabel(cert.status))}</h3><p>${esc(cert.status_detail || "Der aktuelle Bearbeitungsstand wurde vom Server gemeldet.")}</p></div><small>Zuletzt aktualisiert<br><strong>${formatDate(cert.updated_at)}</strong></small></div><ol class="timeline">${["Antrag erfasst", "Schlüssel & ACME-Auftrag", "Domainprüfung", "Ausstellung & Bereitstellung"].map((label, index) => `<li class="${index < stage ? "done" : index === stage ? "current" : ""}"><b>${index < stage ? "✓" : index + 1}</b><span>${label}</span></li>`).join("")}</ol><dl class="detail-list"><dt>Zertifizierungsstelle</dt><dd>${esc(cert.ca_name || cert.acme_directory)}</dd><dt>SANs</dt><dd>${esc(cert.sans.join(", ") || "–")}</dd><dt>Challenge</dt><dd>${esc(cert.challenge)}</dd><dt>Schlüsselmodus</dt><dd>${esc(cert.key_mode)}</dd><dt>Zielsysteme</dt><dd>${cert.targets.map(item => esc(item.name)).join(", ") || "Noch nicht zugewiesen"}</dd><dt>Gültig bis</dt><dd>${formatDate(cert.not_after)}</dd><dt>Erstellt</dt><dd>${formatDate(cert.created_at)}</dd></dl>${["admin", "operator"].includes(state.me.role) ? `<div class="modal-actions"><button class="button" data-download="${id}" ${cert.status !== "issued" ? "disabled title=\"Erst nach Ausstellung verfügbar\"" : ""}>PEM herunterladen</button><button class="button" data-csr="${id}">CSR ersetzen</button></div>` : ""}`);
}
...
async function authorities() {
  state.caAccounts = await api("/api/v1/ca-accounts");
  const action = state.me.role === "admin" ? `<button class="button primary" data-action="new-authority">＋ CA-Anbindung</button>` : "";
  $("#content").innerHTML = `${intro("Zertifizierungsstellen", "ACME-Konten, Kontaktadressen und Nutzungsbedingungen zentral verwalten.", action)}<div class="info-box page-info"><strong>Warum ist das erforderlich?</strong><span>Vor einer Zertifikatsanfrage benötigt die CA eine gültige Kontaktadresse und Ihre Zustimmung zu den jeweils verlinkten Bedingungen. Certify speichert den Zustimmungszeitpunkt revisionsfähig.</span></div><section class="panel"><div class="table-wrap">${state.caAccounts.length ? `<table class="data-table"><thead><tr><th>Name</th><th>Anbieter</th><th>Kontakt</th><th>Bedingungen</th><th>Status</th><th></th></tr></thead><tbody>${state.caAccounts.map(item => `<tr><td><div class="cell-main">${esc(item.name)}</div><div class="cell-sub">${esc(item.directory_url)}</div></td><td>${item.provider === "letsencrypt" ? "Let's Encrypt" : "Eigene ACME-CA"}</td><td>${esc(item.email)}</td><td>${item.terms_url ? `<a href="${esc(item.terms_url)}" target="_blank" rel="noreferrer">Ansehen ↗</a>` : "Nicht verlinkt"}<div class="cell-sub">Akzeptiert ${formatDate(item.terms_accepted_at)}</div></td><td>${badge(item.enabled ? "active" : "inactive")}</td><td>${state.me.role === "admin" && item.enabled ? `<button class="small-button" data-disable-ca="${item.id}">Deaktivieren</button>` : ""}</td></tr>`).join("")}</tbody></table>` : empty("Noch keine CA angebunden", "Legen Sie ein ACME-Konto an, bevor Sie Zertifikate anfordern.")}</div></section>`;
}
function newAuthority() {
  openModal("CA-Anbindung einrichten", "ACME-Konto", `<form id="authority-form"><div class="info-box"><strong>Let's Encrypt oder eigene CA</strong><span>Die Kontaktadresse wird von der CA für wichtige Hinweise zu Konto und Zertifikaten verwendet.</span></div><div class="grid-form"><label>Name<input name="name" required placeholder="Let's Encrypt Produktion"></label><label>Anbieter<select name="provider"><option value="letsencrypt">Let's Encrypt</option><option value="custom">Eigene ACME-CA</option></select></label><label class="full">ACME-Verzeichnis<input name="directory_url" type="url" required value="https://acme-v02.api.letsencrypt.org/directory"></label><label class="full">Kontakt-E-Mail<input name="email" type="email" required placeholder="pki@example.org"></label><label class="full">Link zu den Nutzungsbedingungen<input name="terms_url" type="url" value="https://letsencrypt.org/repository/"></label><label class="full consent"><input name="terms_accepted" type="checkbox" required> Ich habe die verlinkten Bedingungen gelesen und bin berechtigt, sie für die Organisation zu akzeptieren.</label></div><div class="modal-actions"><button type="button" class="button" data-close>Abbrechen</button><button class="button primary">Anbindung speichern</button></div></form>`);
  const form = $("#authority-form"); form.elements.provider.onchange = event => { if (event.target.value === "letsencrypt") { form.elements.directory_url.value = "https://acme-v02.api.letsencrypt.org/directory"; form.elements.terms_url.value = "https://letsencrypt.org/repository/"; } };
  form.onsubmit = async event => { event.preventDefault(); const raw = formJson(form); raw.terms_accepted = form.elements.terms_accepted.checked; raw.terms_url ||= null; try { await api("/api/v1/ca-accounts", { method: "POST", body: JSON.stringify(raw) }); closeModal(); toast("CA-Anbindung wurde eingerichtet."); navigate("authorities"); } catch (error) { toast(error.message, true); } };
}
...
const renderers = { dashboard, certificates, authorities, targets, users, "api-keys": apiKeys, notifications, audit: auditPage, settings };
document.addEventListener("click", async event => {
  const action = event.target.closest("[data-action]")?.dataset.action;
  if (action === "new-certificate") newCertificate(); if (action === "new-authority") newAuthority(); if (action === "new-target") newTarget(); if (action === "new-user") newUser(); if (action === "new-key") newKey(); if (action === "refresh") navigate(location.hash.slice(1));
  ...
  const disableCa = event.target.closest("[data-disable-ca]")?.dataset.disableCa; if (disableCa && confirm("Diese CA-Anbindung für neue Anfragen deaktivieren?")) { try { await api(`/api/v1/ca-accounts/${disableCa}`, { method: "DELETE" }); toast("CA-Anbindung wurde deaktiviert."); navigate("authorities"); } catch (error) { toast(error.message, true); } }
